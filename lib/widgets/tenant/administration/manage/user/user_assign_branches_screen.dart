...
class UserAssignBranchesScreen extends StatefulWidget {
  @override
  _UserAssignBranchesScreenState createState() =>
      _UserAssignBranchesScreenState();
}

class _UserAssignBranchesScreenState extends State<UserAssignBranchesScreen> {
  ...
  void didChangeDependencies() {
    ...
    _getUserAssignBranchesList();
    super.didChangeDependencies();
  }

  Future<List> _getUserAssignBranchesList() async {
    userId = arguments['id'];
    userName = arguments['displayName'];
    if (_userAssignbranchesList.isEmpty) {
      _userAssignbranchesList =
          await _userProvider.getUserAssignedBranches(userId);
    }
    setState(() {
      _isLoading = false;
    });
    return _userAssignbranchesList;
  }

  Future<void> _onSubmit() async {
    try {
      if (_userAssignbranchesList.isNotEmpty) {
        for (int i = 0; i <= _userAssignbranchesList.length - 1; i++) {
          _assignBranches.add(_userAssignbranchesList[i]['id']);
        }
      }
      await _userProvider.assignBranches(
        userId,
        _assignBranches,
      );
                    ...
                    .map(
                      (e) => Chip(
                        ...
                        onDeleted: () {
                          setState(
                            () {
                              _userAssignbranchesList.removeWhere(
      ...
      child: AutocompleteFormField(
        ...
        autocompleteCallback: (pattern) async {
          ...
          if (_userAssignbranchesList.isNotEmpty) {
            for (int i = 0; i <= _userAssignbranchesList.length - 1; i++) {
              data.removeWhere(
                (elm) => elm['id'] == _userAssignbranchesList[i]['id'],
              );
            }
          }
          return data;
        ...
        onSelected: (val) {
          setState(() {
            _userAssignbranchesList.add(val);
          });
