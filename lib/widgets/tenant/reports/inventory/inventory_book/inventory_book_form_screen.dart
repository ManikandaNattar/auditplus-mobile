...
class InventoryBookFormScreen extends StatefulWidget {
  @override
  _InventoryBookFormScreenState createState() =>
      _InventoryBookFormScreenState();
}

class _InventoryBookFormScreenState extends State<InventoryBookFormScreen> {
  ...
  void _getFormData() {
    ...
    _inventoryTextEditingController.text = arguments['inventory'] == ''
        ? _inventoryTextEditingController.text
        : arguments['inventory']['name'];
    _inventoryTextEditingController.selection = TextSelection.fromPosition(
      TextPosition(
        offset: _inventoryTextEditingController.text.length,
      ),
    );
  ...
  Widget _inventoryBookFormDateContainer() {
    return Container(
  ...
  Widget _inventoryBookFormInventoryContainer() {
    return Container(
      width: double.infinity,
      child: Card(
        elevation: 10.0,
          ...
          child: Column(
            ...
            children: [
              ...
              AutocompleteFormField(
                initialValue:
                    utils.cast<Map<String, dynamic>>(arguments['inventory']),
                autoFocus: false,
                ...
                autocompleteCallback: (pattern) async {
                  return await _inventoryItemProvider.inventoryItemAutoComplete(
                    searchText: pattern,
                    inventoryHeads: _selectedBranchList[0]['id'] == '-1'
                        ? _assignedBranches
                            .where((elm) => true)
                            .map((item) => item['inventoryHead'])
                            .toList()
                        : _selectedBranchList
                            .where((element) => true)
                            .map((e) => e['inventoryHead'])
                            .toList(),
                  );
                },
                validator: (value) {
  ...
  Widget _inventoryBookFormBranchContainer() {
    return Container(
      width: double.infinity,
      child: Card(
        elevation: 10.0,
      ...
      body: SingleChildScrollView(
        child: Container(
          ...
          child: Form(
            ...
            child: Column(
              children: [
                _inventoryBookFormDateContainer(),
                _inventoryBookFormInventoryContainer(),
                _inventoryBookFormBranchContainer(),
              ],
            ),
          ),
        ),
